...
WITH base AS (
  ...
  SELECT
    ...
    , CASE
        WHEN
          co."createDate" IS NOT NULL
          AND co."createDate" != ''
          AND co."createDate" ~ '^[0-9]+$'
        THEN TO_TIMESTAMP(co."createDate", 'YYYYMMDDHH24MISS')
      END AS "createDate"
    , co.flow
    , co.price
    , co."finishDate"
    , CASE
        WHEN
          co."updateDate" IS NOT NULL
          AND co."updateDate" != ''
          AND co."updateDate" ~ '^[0-9]+$'
        THEN TO_TIMESTAMP(co."updateDate", 'YYYYMMDDHH24MISS')
      END AS "updateDate"
    , co.updator
    , co."storeId"
    , co."disName"
