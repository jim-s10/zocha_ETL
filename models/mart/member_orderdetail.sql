...
WITH base AS (
    SELECT
        CAST(ioa."clientOrderId" AS VARCHAR(45)) AS "order"
        , CAST(ioa."clientUserId" AS VARCHAR(20)) AS member_id
        , CAST(ioa.partition AS INTEGER) AS "partition"
        , CAST(ioa."isPbgnProd" AS VARCHAR(10)) AS is_pbgn_prod -- 車種類型
        , CAST(ioa.prod_cat_name AS VARCHAR(20)) AS prod_cat_name -- 租用車款
        , CAST(ioa.renting_period AS DECIMAL(20,3)) AS renting_period -- 實際租借時長
        , CAST(ioa.renting_type AS VARCHAR(10)) AS renting_type -- 租借方案(短租/長租)
        , CAST(ioa.store_type AS VARCHAR(10)) AS store_type -- 店家類型(無人/門市)
        ...
        , CAST(ioa."usePurpose" AS VARCHAR(10)) AS reason_to_use -- 預約原因
        , CAST(ioa."sDate" AS TIMESTAMP) AS renting_start_time -- 租車開始時間
        , CAST(ioa."eDate" AS TIMESTAMP) AS renting_end_time -- 租車結束時間
